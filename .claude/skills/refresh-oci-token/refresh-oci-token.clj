...
;;
;; The local OCI profile is session-token based — .envrc sets
;; OCI_CLI_AUTH=security_token because the `oci` CLI rejects that profile
;; otherwise — and Oracle caps a session at 60 minutes. A stale token surfaces
;; as an auth failure at plan time, not as a config error, so `./green create`
;; gets most of the way in before it fails. This is the shortest path back.
;;
