...
./green describe                                  # read-only, exercises the real credentials
```
...
Only `create`/`delete` need credentials at all — `./green build` and
`--dry-run` render from desired state and work fine with a dead token.
